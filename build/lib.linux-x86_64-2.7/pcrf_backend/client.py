...
class PCRFClient(object):
    ...
    def query_session(self, subscriber_id):
        ...
        if capped:
            ...
            #services = [service[0] for service in capped] in capped]
            latest = ', '.join(capped)
    ...
    @xml_error_handler
    @timeout_handler
    def update_session(self, ipAddress):
        keyfield = self.client.factory.create('ns0:KeyFieldType')
        keyfield.code = "framedIp"
        keyfield.value = ipAddress
        key = self.client.factory.create('ns0:SessionKeyType')
        key.code = "FramedIpKey"
        key.primary = "true"
        key.keyField = [keyfield]
        deletedAvp1 = self.client.factory.create('ns0:AvpType')
        deletedAvp1.code = "redirection1"
        deletedAvp1.value = "active_redirect"
        deletedAvp2 = self.client.factory.create('ns0:AvpType')
        deletedAvp2.code = "redirection2"
        deletedAvp2.value = "active_redirect"
        deletedAvp3 = self.client.factory.create('ns0:AvpType')
        deletedAvp3.code = "redirection3"
        deletedAvp3.value = "active_redirect"
        deletedAvp4 = self.client.factory.create('ns0:AvpType')
        deletedAvp4.code = "redirection4"
        deletedAvp4.value = "active_redirect"

        response = self.client.service.UpdateSession(key=key,
                                                     deletedAvp=[deletedAvp1, deletedAvp2, deletedAvp3, deletedAvp4])

        if response.errorCode:
            logger.error('Error happened while removing redirection for IP %s, %s', ipAddress, response.errorMessage)
            return {'action_result': False,
                    'action_error_message': 'Error happened while removing redirection, '
                                            '%s.' % response.errorMessage}
        logger.info("Redirection for IP %s was removed successfully", ipAddress)
        return {'action_result': True, 'action_error_message': ""}
