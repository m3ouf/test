...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        if not len(sys.argv) == 3:
            ...
            "usage: python configure_batch_juniper_qos ips.csv")
        juniper_router = JuniperRouter()

        with open(sys.argv[2]) as routers, open("/tmp/configure_batch_juniper_qos_errors.txt", 'w', 1) as errors:
            ...
            for line in routers:
                ...
                try:
                    ...
                    result, err = juniper_router.set_router_qos(ip)
                    if not result:
                        errors.write(ip + ": " + err + "\n")
